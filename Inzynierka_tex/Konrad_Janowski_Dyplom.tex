...
\chapter{Sposób użycie i testy}
Ta część dotyczyć będzie sposobu użycia programu z punktu widzenia użytkownika oraz przeprowadzonych przez autora testów w celu weryfikacji poprawności działania.
\section{Sposób użycia}
Jak wspomniano w rozdziale \ref{sposób} program działa jako aplikacja konsolowa. Oznacza to, że wywołanie następuje poprzez wiersz poleceń systemu operacyjnego. Rozpatrzmy dokładniej wywołanie programu pokazane na rysunku \ref{cmd2}.
...
Podczas normalnego działania program wypisuje do wiersza poleceń informacje o stanie jego działania. W razie błędu pomaga to zorientować się co go powoduje. Na rysunku \ref{poprawne_dzialanie} możemy zobaczyć przykład poprawnego działania programu a na rysunku \ref{bład} przykład wypisania informacji o błędzie. 

\begin{figure}[hbtp]
\label{poprawne_dzialanie}
\caption{Przykład wiersza poleceń podczas poprawnego działania programu}
\centering
\includegraphics[scale=1]{poprawne_dzialanie.PNG}
\end{figure}
